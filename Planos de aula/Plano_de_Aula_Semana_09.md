...
- IA de utilidade (seção 6.7): **considerações** normalizadas (saúde, distância ao inimigo, munição disponível), **curvas de utilidade** que transformam cada consideração em pontuação parcial, **combinação** das parciais em uma utilidade final por ação e **seleção** da ação de maior valor — gulosa ou probabilística, esta última para evitar comportamento repetitivo; a forma da curva (linear, acelerada, em degrau) como principal alavanca de design da técnica;
- articulação entre as duas técnicas da semana: a IA de utilidade generaliza para a escolha de **ações** a mesma lógica de combinação ponderada que o mapa de influência aplica à escolha de **posições** (retomando a nota da seção 10.2.2 sobre a proximidade entre elas), e o quadro da seção 6.8 situa ambas frente às arquiteturas discretas do Módulo 1; ausência de solução oficial equivalente na Unity em ambos os casos, implementação própria em C#;
