...
9. **Consolidar**, em grupo, o Micro Game Board Game AI, integrando função heurística de avaliação e poda alfa-beta à implementação do Minimax iniciada na Semana 10, permitindo jogar o Conecta 4 Reduzido.
...
| **Cena Unity utilizada** | A mesma cena mínima do Micro Game Board Game AI iniciada na Semana 10, estendida para **Conecta 4 Reduzido** (grade menor que o padrão) — o exemplo fixado para o Módulo 4 —, no qual a exploração completa da árvore deixe de ser viável |
...
| 4. Desafio de Escolha Tecnológica do Módulo 4 | 15 min | Desenvolver a capacidade de justificar tecnicamente uma escolha de busca adversarial | Apresenta um cenário não idêntico ao Micro Game (por exemplo, um jogo de tabuleiro com fator de ramificação bem maior que o jogo da velha, ou um jogo com elemento de acaso) e solicita que cada grupo avalie se o Minimax com alfa-beta é adequado; antes da análise, recapitula brevemente as limitações do Minimax já discutidas nesta semana (explosão combinatória, dependência de heurística de qualidade e de profundidade alcançável) e as alternativas mencionadas no Encerramento da Parte V (Tabela Comparativa Minimax × Alfa-Beta × MCTS) | Analisam o cenário, comparam alternativas com base em requisitos e limitações (fator de ramificação, disponibilidade de heurística, tempo real versus turnos, presença de acaso), e registram a justificativa por escrito |
...
- a IA jogando no **Conecta 4 Reduzido** (grade menor que o padrão), dentro de um tempo de resposta aceitável por jogada;
...
1. Implementar o tabuleiro do **Conecta 4 Reduzido** (grade menor que o padrão), no qual a exploração completa da árvore não seja mais viável em tempo aceitável.
2. Definir de duas a quatro características (features) da posição relevantes para o Conecta 4 Reduzido, atribuir pesos e implementar a função de avaliação como soma ponderada, aplicada nos nós da fronteira de profundidade.
