...
5. Montar, na Unity, o cenário mínimo correspondente (agente, ambiente, alvo/obstáculos).
6. **Escrever o esqueleto do script de `Agent` em C#**, traduzindo o planejamento em código: os métodos de coleta de observações, de execução de ações e de atribuição de recompensa, ainda **sem** configurar os componentes de ML-Agents nem iniciar qualquer treinamento. O objetivo aqui é conceitual — tornar visível que estado, ação e recompensa viram três métodos —, e não operar a ferramenta.
7. Apresentar a proposta de estado, ação e recompensa na discussão técnica do Encontro 2, recebendo retorno para ajuste antes da Semana 15.
